...
#    x = tf.unstack(x, seq_length, 1)
    forward_lstm_cell = rnn_cell.BasicLSTMCell(rnn_size,state_is_tuple=True)
    backward_lstm_cell = rnn_cell.BasicLSTMCell(rnn_size,state_is_tuple=True)
    outputs, _, _= tf.nn.static_bidirectional_rnn(forward_lstm_cell, backward_lstm_cell, x, dtype=tf.float32)
